...
/// Abstraction over power data collection (macmon subprocess).
/// Implementations: `MacMonService` (production), `MockPowerMonitor` (tests).
protocol PowerMonitoring: AnyObject {
    /// Latest metrics from the power data source. `nil` until first reading.
    var metricsPublisher: AnyPublisher<PowerMetrics?, Never> { get }
